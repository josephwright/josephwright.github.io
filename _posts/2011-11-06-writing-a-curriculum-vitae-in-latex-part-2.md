...
That sets up the fonts, but what about page layout? Well, the standard here is to use the [geometry](http://ctan.org/pkg/geometry) package
...
```
at the end of my package-loading section. The [hyperref](http://ctan.org/pkg/hyperref) package deals with the links, while the set up makes them blend in to the text and adds my name to the PDF information.
...
(this uses the [array](http://ctan.org/pkg/array) package). I also `\renewcommand*\arraystretch{1.4}`, as this spreads the tables out a bit and I think makes things look less crowded.
