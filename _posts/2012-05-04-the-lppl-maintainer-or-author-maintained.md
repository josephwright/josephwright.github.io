...
Packages which are either ‘maintained’  or  ‘author-maintained’ have one or more people looking after it, and they are responsible for making changes to the code. The difference comes if those people disappear (as has [happened recently with biblatex](http://www.texdev.net/2012/04/03/biblatex-status/)). With a ‘maintained’ package, it's possible for a new person or team to make a public statement that they are going to take over, then after a delay they become the new maintainers. On the other hand, a package which is ‘author-maintained’ cannot be taken over by someone else.

Now, the LPPL is a ‘free’ license and so it is always possible to create a fork from an existing package. In general, you don't really want to do that simply to keep updating an existing package: taking over maintenance is much clearer all round. For biblatex, we can't do that as it's ‘author-maintained’. So we're going to have to [formally fork the project](http://www.texdev.net/2012/04/23/biblatex-a-team-to-continue-the-work/), mark the ‘new’ version as distinct from the old (Philipp Lehman) version and ensure that both remain on CTAN: not ideal.
